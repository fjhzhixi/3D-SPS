...
class ScannetDatasetConfig(object):
    def __init__(self):
        ...
        if CONF.det_class_label == 'all':
            ...
            self.type2class = self._get_type2class_all()
            self.class2type = {self.type2class[t]:t for t in self.type2class}
        elif CONF.det_class_label == 'main_with_others':
            self.type2class = {'cabinet':0, 'bed':1, 'chair':2, 'sofa':3, 'table':4, 'door':5,
                'window':6,'bookshelf':7,'picture':8, 'counter':9, 'desk':10, 'curtain':11,
                'refrigerator':12, 'shower curtain':13, 'toilet':14, 'sink':15, 'bathtub':16, 'others':17}  
            self.class2type = {self.type2class[t]:t for t in self.type2class}
        ...
        if CONF.det_class_label == 'all':
            other_mean_size = np.array([self.mean_size_arr[-1]])
            for _ in range(len(self.nyu40ids) - self.mean_size_arr.shape[0]):
                self.mean_size_arr = np.append(self.mean_size_arr, other_mean_size, 0)
